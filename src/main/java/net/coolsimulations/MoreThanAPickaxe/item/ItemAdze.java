...
public class ItemAdze extends ItemTool{
	...
	protected static final Map<Block, IBlockState> BLOCK_PATH_MAP = Maps.newHashMap(ImmutableMap.of(Blocks.GRASS_BLOCK, Blocks.GRASS_PATH.getDefaultState()));
	protected static final Map<Block, IBlockState> BLOCK_HOE_MAP = Maps.newHashMap(ImmutableMap.of(Blocks.GRASS_BLOCK, Blocks.FARMLAND.getDefaultState(), Blocks.GRASS_PATH, Blocks.FARMLAND.getDefaultState(), Blocks.DIRT, Blocks.FARMLAND.getDefaultState(), Blocks.COARSE_DIRT, Blocks.DIRT.getDefaultState()));
	   ...
	   if(!context.isPlacerSneaking()) {
		   ...
		   int hook = net.minecraftforge.event.ForgeEventFactory.onHoeUse(context);
		      ...
		      if (context.getFace() != EnumFacing.DOWN && world.isAirBlock(blockpos.up())) {
		         IBlockState iblockstate2 = BLOCK_HOE_MAP.get(world.getBlockState(blockpos).getBlock());
		   ...
		   if (context.getFace() != EnumFacing.DOWN && world.getBlockState(blockpos.up()).isAir()) {
	         	IBlockState iblockstate1 = BLOCK_PATH_MAP.get(world.getBlockState(blockpos).getBlock());
    ...
    /**public boolean canHarvestBlock(IBlockState blockIn)
    {
        ...
        if (block == Blocks.SNOW_BLOCK){
        	return block == Blocks.SNOW_BLOCK ? true : block == Blocks.SNOW;
        }
